...
public class UHFReader {
    ...
    public synchronized UHFReaderResult<Boolean> connect(Context context) {
        if(getConnectState() == ConnectState.CONNECTED){
            return new UHFReaderResult(UHFReaderResult.ResultCode.CODE_SUCCESS,"模块已经连接成功,请勿重复连接!",true);
        }
    ...
    public synchronized UHFReaderResult<Boolean> disConnect() {
        ...
        if(getConnectState() != ConnectState.CONNECTED){
            return new UHFReaderResult(UHFReaderResult.ResultCode.CODE_READER_NOT_CONNECTED, UHFReaderResult.ResultMessage.READER_NOT_CONNECTED,false);
        }
...
//        }
    ...
    public UHFReaderResult<UHFTagEntity> singleTagInventory(SelectEntity selectEntity) {
        ...
        if(selectEntity!=null){
            if(selectEntity.getLength()==0){
                return new UHFReaderResult<UHFTagEntity>(UHFReaderResult.ResultCode.CODE_FAILURE,"选择的标签长度不能为0!",null);
            }
            if(selectEntity.getData()==null){
                return new UHFReaderResult<UHFTagEntity>(UHFReaderResult.ResultCode.CODE_FAILURE,"选择的标签数据不能为null!",null);
            }
        }
...
//TODO
    ...
    public UHFReaderResult<Integer> getTemperature() {
        if(getConnectState() != ConnectState.CONNECTED){
            return new UHFReaderResult(UHFReaderResult.ResultCode.CODE_READER_NOT_CONNECTED, UHFReaderResult.ResultMessage.READER_NOT_CONNECTED,0);
        }
    ...
    public UHFReaderResult<String> read(String password,int membank,int address,int wordCount,SelectEntity selectEntity) {
        if(getConnectState() != ConnectState.CONNECTED){
            return new UHFReaderResult<String>(UHFReaderResult.ResultCode.CODE_READER_NOT_CONNECTED, UHFReaderResult.ResultMessage.READER_NOT_CONNECTED,null);
        }
        if(selectEntity!=null){
            if(selectEntity.getLength()==0){
                return new UHFReaderResult<String>(UHFReaderResult.ResultCode.CODE_FAILURE,"选择的标签长度不能为0!",null);
            }
            if(selectEntity.getData()==null){
                return new UHFReaderResult<String>(UHFReaderResult.ResultCode.CODE_FAILURE,"选择的标签数据不能为null!",null);
            }
        }
    ...
    public UHFReaderResult<Boolean> write(String password, int membank, int address, int wordCount, String data, SelectEntity selectEntity) {
        if(getConnectState() != ConnectState.CONNECTED){
            return new UHFReaderResult<Boolean>(UHFReaderResult.ResultCode.CODE_READER_NOT_CONNECTED, UHFReaderResult.ResultMessage.READER_NOT_CONNECTED,false);
        }
        if(selectEntity!=null){
            if(selectEntity.getLength()==0){
                return new UHFReaderResult(UHFReaderResult.ResultCode.CODE_FAILURE,"选择的标签长度不能为0!",null);
            }
            if(selectEntity.getData()==null){
                return new UHFReaderResult(UHFReaderResult.ResultCode.CODE_FAILURE,"选择的标签数据不能为null!",null);
            }
        }
